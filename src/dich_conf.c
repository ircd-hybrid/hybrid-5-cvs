...
/*
  To understand this code, read Knuth, Sorting and Searching, third volume
  of "The Art Of Computer Programming" pg. 472
  dich_conf.c is an implementation of multi-way trees, sometimes referred
  to as "B*" or "B+" tree's
  In this implementation, the host part forms a multi-way tree, with the
  bottom most leaf pointing to a link list of aConfItem items, each
  corresponding to an user. Note: Soleil is doing his own block allocator.
...
WARNING!!!!
  ...
  If klines/dlines etc. appear to be "garbled" i.e. repeated over
and over again, chances are you are using the wrong version of memmove
...
/*
A faster form of reverse than the original, yes I wrote this
ages ago. 
...
/*  if( !strchr(my_string, '*')) */
    				/* Its sortable, as is, without reverse()
				   This will save even further time
				   as Sol's original code would return -1
				   (needed reversing) -Dianora */
/*    return(1); */
...
/*	    return(-1); */	/* what sol's orig  returns, but 1 will be */
...
void addto_conf_list(aConfList *my_list,
		aConfItem *my_conf,
		char *(*cmp_field)() )
{
